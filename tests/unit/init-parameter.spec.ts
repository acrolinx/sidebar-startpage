...
describe('init-parameters', () => {
  ...
  describe('sanitizeClientComponents', () => {
    ...
    it('use name as id fallback', () => {
      assertDeepEqual(sanitizeClientComponent(_.omit(clientComponent, 'id'), 0).id, clientComponent.name);
    });
    ...
    it('version fallback', () => {
      assertDeepEqual(sanitizeClientComponent(_.omit(clientComponent, 'version'), 0).version, '0.0.0.0');
    });

    it('use id as name fallback', () => {
      assertDeepEqual(sanitizeClientComponent(_.omit(clientComponent, 'name'), 0).name, clientComponent.id);
    });
  });
